...
const rtc_config_t rtc_config =
  { .wakeupSelect = false, .updateMode = false, .supervisorAccess = false,
      .compensationInterval = 0x0U, .compensationTime = 0x0U };

typedef enum AlarmStates_e
{
  ALARM_STOPPED = 0, ALARM_RUNNING = !ALARM_STOPPED
} AlarmStates_t;
 ...
 */
uint32_t
RtcMs2Tick (TimerTime_t milliseconds)
{
  return 0;
}
 ...
 */
void
RtcDelayMs (TimerTime_t milliseconds)
{
  for (;;)
    {
      /*!
...
void
RtcStartAlarm (uint32_t timeout)
{
  rtc_datetime_t date;
...
uint32_t
RtcGetTimerElapsedTime (void)
{
  return (uint32_t) (RTC->TSR - RtcTimerContext.Time);
}

void
RtcBkupWrite (uint32_t data0, uint32_t data1)
{
  //TODO: Implement
...
void
RtcBkupRead (uint32_t *data0, uint32_t *data1)
{
  //TODO: Implement
...
TimerTime_t
RtcTempCompensation (TimerTime_t period, float temperature)
{
  /*!
   * Not implemented yet.
   */
