...
static void
ClearGpioInterruptFlag (GPIO_Type *base, uint32_t mask);

/*!
 * CAUTION: Configurations like Pin-Muxing, Pull-up/Pull-down and Clocks need to be preconfigured in BOARD_InitPins
 */
void
GpioMcuInit (Gpio_t *obj, PinNames pin, PinModes mode, PinConfigs config,
...
void
GpioMcuSetContext (Gpio_t *obj, void *context)
{
  obj->Context = context;
}

void
GpioMcuSetInterrupt (Gpio_t *obj, IrqModes irqMode, IrqPriorities irqPriority,
...
void
GpioMcuRemoveInterrupt (Gpio_t *obj)
{
  if (obj->pin >= PTA_0)
    {
      gpio_pin_config_t gpio_config;
...
#else
      PORT_SetPinInterruptConfig (obj->port, 0x1 << obj->pinIndex,
...
void
GpioMcuWrite (Gpio_t *obj, uint32_t value)
{
  if (obj->pin >= PTA_0)
    {
      if (obj == NULL)
