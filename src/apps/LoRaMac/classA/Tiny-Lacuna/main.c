...
LoRaMacHandlerAppData_t AppData = { .MsgType = LORAMAC_HANDLER_UNCONFIRMED_MSG,
		.Buffer = NULL, .BufferSize = 0, .Port = 0 };
 ...
 * MAC status strings
 */
const char *MacStatusStrings[] = { "OK",                    // LORAMAC_STATUS_OK
 ...
 */
const char *EventInfoStatusStrings[] = { "OK",   // LORAMAC_EVENT_INFO_STATUS_OK
		...
		case DEVICE_STATE_RESTORE: {
			...
			if (NvmCtxMgmtRestore() == NVMCTXMGMT_STATUS_SUCCESS) {
				...
				if ((devEui[0] == 0) && (devEui[1] == 0) && (devEui[2] == 0)
						&& (devEui[3] == 0) && (devEui[4] == 0)
						&& (devEui[5] == 0) && (devEui[6] == 0)
						&& (devEui[7] == 0)) {
